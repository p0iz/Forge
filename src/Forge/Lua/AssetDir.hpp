...
class AssetDir
{
  public:
    ...
    void load(lua_State* state)
    {
      ...
      if (mNumEntries == 0)
      {
        AssetLibrary<AssetType, AssetTraits<AssetType>::isRefCounted>::instance().addDirectory(lua_tostring(state, -1));
      }
      else
      {
      ...
      AssetLibrary<AssetType, AssetTraits<AssetType>::isRefCounted>::instance().loadAssetInfo();
      lua_pop(state, 1);
    }
